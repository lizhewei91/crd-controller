...
type UnitedDeploymentLister interface {
	...
	List(selector labels.Selector) (ret []*v1.UnitedDeployment, err error)
...
// List lists all UnitedDeployments in the indexer.
func (s *unitedDeploymentLister) List(selector labels.Selector) (ret []*v1.UnitedDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.UnitedDeployment))
	})
	return ret, err
}
...
type UnitedDeploymentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.UnitedDeployment, error)
...
// List lists all UnitedDeployments in the indexer for a given namespace.
func (s unitedDeploymentNamespaceLister) List(selector labels.Selector) (ret []*v1.UnitedDeployment, err error) {
...
// Get retrieves the UnitedDeployment from the indexer for a given namespace and name.
func (s unitedDeploymentNamespaceLister) Get(name string) (*v1.UnitedDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("uniteddeployment"), name)
	}
	return obj.(*v1.UnitedDeployment), nil
}
